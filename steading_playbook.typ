...
#let steading_name = "Lygosian Community"
#let SPREADS = true
  ...
  #grid(
    ...
    grungebox(stroke: black, inset: 1em, radius: 1em)[
      ...
      #checklist[
        ...
        - Institution (integration into the state)
      ]
...
#let steading_map_page = [
  ...
  #grid(
    ...
    grungebox(width: 100%, height: 5.5cm, stroke: black, inset: 1em, radius: 1em)[
      = Notes
      #set text(font: "Crafty Girls", size: 6.5pt)
...
#let steading_improvements = (
  ...
  [
    ...
    *Requires* all of the following, in order:
    ...
    Henceforth, when *_the seasons change_*, whoever is friendliest rolls +Fortunes: on a 10+, ask the GM 3 questions about the wider world; on a 7-9, ask 1 question; on a 6-, ask 1 question, but the GM describes some trouble that stems from the inn or its guests.

    Once per season, when you expend 1 Surplus and bring folks together at the temple (to talk, to celebrate, to recuperate), clear one of the steading's debilities.
  ],
  [
    == Mandate of Heaven
  ...
  [
    ...
    *Requires*:
    ...
    When you *_meet the requirements_*, #lorem(40)
  ],
  [
    == Gymnasium
    #lorem(60)
...
#let format_improvements(improvements) = {
  for (idx, improvement) in improvements.enumerate() {
    edge_and_count_aware(idx, "improvement", checkblock(improvement), stop_state: stop_state)
  }
...
#let improvements_pages = (
  [
    #big_line() <hydaelyn>
    #columns(2, format_improvements(steading_improvements))
  ],
  [],
  [],
)
